...
ifeq ($(NO_DOCKER), 1)
  ...
  DOCKER_CMD := $(RUNTIME) run --rm -e CGO_ENABLED=1 -v "$(PWD):/go/src/$(REPO_PATH):Z" -w "/go/src/$(REPO_PATH)" openshift/origin-release:golang-1.13
  IMAGE_BUILD_CMD = $(RUNTIME) build
endif
...
push:
	...
	$(RUNTIME) push "$(IMAGE):$(MUTABLE_TAG)"
